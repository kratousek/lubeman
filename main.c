...
int main(int argc, char *argv[])
{
   ...
   if (msgsnd(msqid, &sbuf, buflen, IPC_NOWAIT)  < 0)
      logPrintf("Cannot send STA do guardian\r\n");
...
check_server_connection:
    ...
    buflen=strlen(sbuf.mtext)+1;
    if (msgsnd(msqid, &sbuf, buflen, IPC_NOWAIT)  < 0)
      logPrintf("Cannot sent CSR do guardian\r\n");
 
    // reset data
    tcppesResetData(pc.procData);
   
    // check connection
    if(pc.connectionCheck(pc.procData) != 0)
    {
	  blink_interval_us = 500000;
	  goto start_blinking;
    }
    
    // then make sure that database is alive
    // as the test is expensive, do it after 
    if (i_chkdb++>DBCOUNT)
    {
      i_chkdb=0;
      if (tcppesCheckDB(pc.procData) < 0)
      {
        //dataFailure();
	    blink_interval_us = 100000;
        goto start_blinking; 
      }
    }
    ...
    buflen=strlen(sbuf.mtext)+1;
    if (msgsnd(msqid, &sbuf, buflen, IPC_NOWAIT)  < 0)
      logPrintf("Cannot sent SOK do guardian\r\n");
    }
#else
   
    // server OK responding to CHK packet
    sbuf.mtype = 1;
    sprintf(sbuf.mtext,"SOK");
    buflen=strlen(sbuf.mtext)+1;
    if (msgsnd(msqid, &sbuf, buflen, IPC_NOWAIT)  < 0)
...
wait_for_pes:
    ...
    buflen=strlen(sbuf.mtext)+1;
    if (msgsnd(msqid, &sbuf, buflen, IPC_NOWAIT)  < 0)
        logPrintf("Cannot sent CP3 do guardian\r\n");
    ...
    */
    ...
    //logPrintf("main. You have P2/P3 Sensor on TMD: \r\n");
...
// ret =p3_reconnect();
	...
	// if we have zero events in p3, do not read and do not erase p3
    if (firstfree==0)
    { 
		pcBufRead=0;
		goto play_melody; 
...
// set time to P3
    ...
    if (conlost==1)
      goto check_server_connection;    
    goto wait_for_adapter;
   // --------------- END OF THE MAIN LOOP, code below handles connection loss------
   // TADY konci Markova vetev, s nalezenym hardware, serverem i databazi
